...
class ManageUploadCategoryImage extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.CheckSubCategoryResponse) {
      if (
        nextProps.CheckSubCategoryResponse &&
        nextProps.CheckSubCategoryResponse !=
          this.props.CheckSubCategoryResponse
      ) {
        if (nextProps.CheckSubCategoryResponse.data.length > 0) {
          this.setState({ isGettingTags: false });
          this.setState({
            categoryData: nextProps.CheckSubCategoryResponse.data,
          });
        }
      }
    }
  ...
  onSubmit = async (formValues) => {
    ...
    if (this.state.image.length > 0) {
      ...
      for (var i = 0; i < image.length; i++) {
        ...
        var categoryImageId =
          formValues.inCategoryImageId == undefined ||
          formValues.inCategoryImageId == ""
            ? 0
            : formValues.inCategoryImageId;
