...
import {
  cartCountSubscriber,
  cartCountService,
} from "../pagination/dataTransferService";
...
class Profile extends Component {
  ...
  onSubmit = (formValues) => {
    ...
    var userJSON = JSON.parse(localStorage.getItem("persist:v713-demo1-auth"))
      .user;
    var userData = JSON.parse(userJSON);
    var _persist = JSON.parse(localStorage.getItem("persist:v713-demo1-auth"))
      ._persist;
    ...
    var data = {
      inUserId: this.state.currentUserData.inAdminUserId,
      stFirstName: stFirstName,
      stLastName: stLastName,
      stOldPassword:
        formValues.oldPassword != undefined ? formValues.oldPassword : "",
      stNewPassword:
        formValues.newPassword != undefined ? formValues.newPassword : "",
      stConfurmPassword:
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.updateResponse) {
      if (
        nextProps.updateResponse &&
        nextProps.updateResponse != this.props.updateResponse
      ) {
        if (nextProps.updateResponse.statusCode == 200) {
          if (nextProps.updateResponse.data[0].PasswordUpdated == 0) {
            ...
            var userJSON = JSON.parse(
              localStorage.getItem("persist:v713-demo1-auth")
            ).user;
            var userData = JSON.parse(userJSON);
            var _persist = JSON.parse(
              localStorage.getItem("persist:v713-demo1-auth")
            )._persist;
            ...
            var locatusername = JSON.parse(JSON.parse(localdata).user);
            locatusername.token = userData.token;
            locatusername.data.stFirstName =
              nextProps.updateResponse.data[0].stFirstName;
            locatusername.data.stLastName =
              nextProps.updateResponse.data[0].stLastName;
            locatusername.data.stContact =
              nextProps.updateResponse.data[0].stContact;
            locatusername.data.stEmail =
              nextProps.updateResponse.data[0].stEmail;
            let localStorageItems = {
              user: JSON.stringify(locatusername),
              _persist: _persist,
            };
          ...
          this.setState({
            Message:
              nextProps.updateResponse.errorMessages != undefined
                ? nextProps.SaveProfileResponse.errorMessages[0].errorMessage
  ...
  onChange = (event) => {
    ...
    if (event.target.files.length > 0) {
      if (event.target.files[0].size / 1024 / 1024 > 5) {
        this.setState({
          isError: true,
          error: "Insurance card image should be smaller than 5 MB",
        });
      }
      if (event.target.files && event.target.files[0]) {
        ...
        if (type.match(type_reg)) {
          ...
          this.setState({
            isError: true,
            error: "Only file with the .png, .jpeg, .jpg extension are allowed",
          });
        }
      }
      this.setState({
  ...
  removeImage = () => {
    this.setState({
      ...
      DoImageRemove: true,
      stOldImagePath: this.state.imagePath,
      imagePath: "",
    });
  };
  getFiles(files) {
    ...
    if (files.name.length > 0) {
      if (files.size.split(" ")[0] / 1024 > 5) {
        this.setState({
          isError: true,
          error: "image should be smaller than 5 MB",
        });
      }
      ...
      if (
        ...
        this.setState({
          isError: true,
          error:
            "Only file with the .gif, .png, .bmp, .jpeg, .jpg extension are allowed",
        });
      }
    } else {
