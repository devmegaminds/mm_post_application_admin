...
class ManageCategory extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.DeleteCategoryByIdResponse) {
      if (
        nextProps.DeleteCategoryByIdResponse &&
        nextProps.DeleteCategoryByIdResponse !=
          this.props.DeleteCategoryByIdResponse
      ) {
        if (nextProps.DeleteCategoryByIdResponse.data.statusCode == 200) {
          this.props.GetCategory("");
          this.setState({ isGettingTags: false });
          this.SuccessFailSweetAlert(
            nextProps.DeleteCategoryByIdResponse.data.messgae,
            "success"
          );
        }
      }
  ...
  __onChange(e, row) {
    var intIndex = this.state.tagsData
      .map(function(e) {
        return row.inCategoryId;
      })
      .indexOf(row.inCategoryId);
    this.state.tagsData[intIndex].flgIsActive.data[0] = e.target.checked
      ? 1
      : 0;
                ...
                <a
                  ...
                  onClick={(e) =>
                    this.ConfirmationSweetAlert(
                      row,
                      "Do you want to delete this categoy?"
                    )
                  }
                >
